...
import { Item, RecentlyPlayedRoot } from "../interfaces/RecentlyPlayed";
import { RootState } from "../store/root";

type AddHeight = 0 | 210 | 420 | 630

interface Props {
    data: RecentlyPlayedRoot;
    scale: number;
    title: string;
}
...
const TopTracksCanvas: React.FC<Props> = ({ data, title }) => {
    ...
    const darwHearder = () => {
        ...
        if (context) {
            ...
            data.items.map((item, idx) => (count_ms += item.track.duration_ms));
    ...
    const darwBody = (row: number, dataItem: Item) => {
        ...
        if (context) {
            ...
            img.src = dataItem.track.album.images.filter(
                (url) => url.width === 300
            )[0].url;
            ...
            context.fillText(
                limitString(
                    `${row + 1}. ${dataItem.track.name} - ${dataItem.track.artists[0].name}`
                ),
                (50 + x) * scale,
            ...
            context.fillStyle = `#B5B7B5`;
            context.fillText(
                moment.utc(dataItem.track.duration_ms).format("mm:ss"),
                (375 + x) * scale,
                (50 + y) * scale + 42.5 * scale * row
            );
            ...
            context.fillStyle = `#858D8D`;
            context.fillText(
                starCalculator(dataItem.track.popularity),
                (375 + x) * scale,
                (67.5 + y) * scale + 42.5 * scale * row
            );
        }
    ...
    return (
        <div className="flex flex-col  md:flex-row p-5 bg-light gap-3">
            ...
            <div className="flex gap-2 items-center w-full">
                ...
                <div className="flex gap-2 items-center w-full">
                    <div onClick={() => download()} className="btn btn-primary w-full">
                        Download
                    </div>
                </div>
